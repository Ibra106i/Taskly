...
import { SEARCH_MAX, type McpContext } from "@/lib/mcp/schema";
import type { McpServer } from "@modelcontextprotocol/server";

export function registerSearchTodos(server: McpServer, ctx: McpContext) {
  server.registerTool("search_todos", {
    ...
    const { data: todos, error } = await ctx.supabase
      .from("todos").select("*").eq("user_id", ctx.userId)
      .ilike("title", `%${escapeLike(params.query)}%`)
